var DT_OnlinePlacementQuestionList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_OnlinePlacementQuestionList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_OnlinePlacementQuestionList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                { "className": "dt-center", "data": "placementQuestionTypeTitle", "autowidth": true },
                { "className": "dt-center", "data": "placementQuestionTypeCodeName", "autowidth": true },
                {
                    "data": "isActive",
                    "className": "dt-center",
                    "orderable": false,
...

                {
                    ...
                    "orderable": false,
                    render: function (data, type, row) {
                        if (row.placementQuestionTypeCodeName == "CheckBox" || row.placementQuestionTypeCodeName == "RadioButton" || row.placementQuestionTypeCodeName == "ComboBox") {
                            return '<a class="btn btn-warning font-weight-bold mr-2"' +
                                'href="/OnlinePlacement/EditOnlinePlacementQuestion_Load?Id=' + row.id + '" >ویرایش</a> '
                                + '<a class="btn btn-info font-weight-bold mr-2"' +
                                'href="/OnlinePlacement/OnlinePlacementQuestionAnswerList?Id=' + row.id + '" >جواب ها</a>';
                        }
                        else {
                            return '<a class="btn btn-warning font-weight-bold mr-2"' +
                                'href="/OnlinePlacement/EditOnlinePlacementQuestion_Load?Id=' + row.id + '" >ویرایش</a> ';
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_OnlinePlacementQuestionList')) {
        DT_OnlinePlacementQuestionList.init();
    }
});
