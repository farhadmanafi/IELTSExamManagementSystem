var DT_OnlinePlacementList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_OnlinePlacementList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_OnlinePlacementList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-warning font-weight-bold mr-2"' +
                            'href="/OnlinePlacement/EditOnlinePlacement_Load?Id=' + row.id + '" >ویرایش</a> '
                            + '<a class="btn btn-info font-weight-bold mr-2"' +
                                'href="/OnlinePlacement/OnlinePlacementQuestionList?Id=' + row.id + '" >سوالات</a>'
                            + '<a class="btn btn-info font-weight-bold mr-2"' +
                                'href="/OnlinePlacement/PlacementUserScoreList?Id=' + row.id + '" >نتایج</a>';
                    }
                },
                {/*Pricing*/
                    "className": "dt-center",
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-info font-weight-bold mr-2"' +
                            'href="/OnlinePlacement/PlacementExamPrice_Load?Id=' + row.id + '" >مدیریت</a>'
                    }
                },
            ],
            columnDefs: [{
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_OnlinePlacementList')) {
        DT_OnlinePlacementList.init();
    }
});
