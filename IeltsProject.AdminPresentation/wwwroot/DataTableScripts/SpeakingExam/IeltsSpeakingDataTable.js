var DT_IeltsSpeakingList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_IeltsSpeakingList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_IeltsSpeakingList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-warning font-weight-bold mr-2"' +
                            'href="/IeltsExamSpeaking/EditSpeakingExam_Load?Id=' + row.id + '" >ویرایش</a> '
                            //+ '<a class="btn btn-success font-weight-bold mr-2"' +
                            //'href="/IeltsExamSpeaking/IeltsUserScoreList?Id=' + row.id + '" >نتایج</a>'
                            + '<a class="btn btn-success font-weight-bold mr-2"' +
                            'href="/IeltsExamSpeaking/SpeakingMeeting_Load?Id=' + row.id + '" >لیست تایم جلسات</a>';
                    }
                },
            ],
            columnDefs: [{
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_IeltsSpeakingList')) {
        DT_IeltsSpeakingList.init();
    }
});
