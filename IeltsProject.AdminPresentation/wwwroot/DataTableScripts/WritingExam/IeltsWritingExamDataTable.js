var DT_IeltsWritingList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_IeltsWritingList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_IeltsWritingList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-warning font-weight-bold mr-2"' +
                            'href="/IeltsExamWriting/EditWritingExam_Load?Id=' + row.id + '" >ویرایش</a> '
                            //+ '<a class="btn btn-info font-weight-bold mr-2"' +
                            //'href="/IeltsExamWriting/WritingQuestionList?Id=' + row.id + '" >سوالات</a>'
                            //+ '<a class="btn btn-success font-weight-bold mr-2"' +
                            //'href="/IeltsExamWriting/IeltsUserScoreList?Id=' + row.id + '" >نتایج</a>'
                            + '<a class="btn btn-success font-weight-bold mr-2"' +
                            'href="/IeltsExamWriting/WritingSection_Load?Id=' + row.id + '" >لیست بخش ها</a>';
                    }
                },
            ],
            columnDefs: [{
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_IeltsWritingList')) {
        DT_IeltsWritingList.init();
    }
});
