var DT_ReadingQuestionList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_ReadingQuestionList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_ReadingQuestionList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                { "className": "dt-center", "data": "readingQuestionTypeTitle", "autowidth": true },
                { "className": "dt-center", "data": "readingQuestionTypeCodeName", "autowidth": true },
                {
                    "data": "isActive",
                    "className": "dt-center",
                    "orderable": false,
                ...
                },
                {
                    ...
                    "orderable": false,
                    render: function (data, type, row) {
                        if (row.readingQuestionTypeCodeName == "CheckBox" || row.readingQuestionTypeCodeName == "RadioButton" || row.readingQuestionTypeCodeName == "ComboBox") {
                            return '<a class="btn btn-warning font-weight-bold mr-2"' +
                                'href="/IeltsExamReading/EditReadingQuestion_Load?Id=' + row.id + '" >ویرایش</a> '
                                + '<a class="btn btn-info font-weight-bold mr-2"' +
                                'href="/IeltsExamReading/ReadingQuestionAnswerList?Id=' + row.id + '" >جواب ها</a>';
                        }
                        else {
                            return '<a class="btn btn-warning font-weight-bold mr-2"' +
                                'href="/IeltsExamReading/EditReadingQuestion_Load?Id=' + row.id + '" >ویرایش</a> ';
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_ReadingQuestionList')) {
        DT_ReadingQuestionList.init();
    }
});
