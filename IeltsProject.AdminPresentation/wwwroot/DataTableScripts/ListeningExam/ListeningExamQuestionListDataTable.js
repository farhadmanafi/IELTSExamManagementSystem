var DT_ListeningQuestionList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_ListeningQuestionList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_ListeningQuestionList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                { "className": "dt-center", "data": "listeningQuestionTypeTitle", "autowidth": true },
                { "className": "dt-center", "data": "listeningQuestionTypeCodeName", "autowidth": true },
                {
                    "data": "isActive",
                    "className": "dt-center",
                    "orderable": false,
                ...
                },
                {
                    ...
                    "orderable": false,
                    render: function (data, type, row) {
                        if (row.listeningQuestionTypeCodeName == "CheckBox" || row.listeningQuestionTypeCodeName == "RadioButton" || row.listeningQuestionTypeCodeName == "ComboBox") {
                            return '<a class="btn btn-warning font-weight-bold mr-2"' +
                                'href="/IeltsExamListening/EditListeningQuestion_Load?Id=' + row.id + '" >ویرایش</a> '
                                + '<a class="btn btn-info font-weight-bold mr-2"' +
                                'href="/IeltsExamListening/ListeningQuestionAnswerList?Id=' + row.id + '" >جواب ها</a>';
                        }
                        else {
                            return '<a class="btn btn-warning font-weight-bold mr-2"' +
                                'href="/IeltsExamListening/EditListeningQuestion_Load?Id=' + row.id + '" >ویرایش</a> ';
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_ListeningQuestionList')) {
        DT_ListeningQuestionList.init();
    }
});
