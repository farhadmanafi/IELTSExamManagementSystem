var DT_IeltsListeningList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_IeltsListeningList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_IeltsListeningList').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-warning font-weight-bold mr-2"' +
                            'href="/IeltsExamListening/EditListeningExam_Load?Id=' + row.id + '" >ویرایش</a> '
                            + '<a class="btn btn-info font-weight-bold mr-2" target="_blank"' +
                            'href="/IeltsExamListening/PlayListeningQuestionVoice?Id=' + row.id + '" >پخش صوت</a>'
                            + '<a class="btn btn-info font-weight-bold mr-2"' +
                            'href="/IeltsExamListening/ListeningQuestionList?Id=' + row.id + '" >سوالات</a>'
                            //+ '<a class="btn btn-success font-weight-bold mr-2"' +
                            //'href="/IeltsExamListening/IeltsUserScoreList?Id=' + row.id + '" >نتایج</a>'
                            + '<a class="btn btn-success font-weight-bold mr-2"' +
                            'href="/IeltsExamListening/ListeningSection_Load?Id=' + row.id + '" >لیست بخش ها</a>';
                    }
                },
            ],
            columnDefs: [{
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_IeltsListeningList')) {
        DT_IeltsListeningList.init();
    }
});
