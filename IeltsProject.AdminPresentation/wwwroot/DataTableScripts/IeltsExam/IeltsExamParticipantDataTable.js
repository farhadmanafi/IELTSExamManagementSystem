var DT_IeltsExamParticipants = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_IeltsExamParticipants').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_IeltsExamParticipants').data("url"),
                type: 'POST'
            },
            columns: [
                ...
                { "className": "dt-center", "data": "registerDatePersian", "autowidth": true },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-warning font-weight-bold mr-2"' +
                            'href="/IeltsExamResult/IeltsExamParticipantResult_Load?customerId=' + row.customerId + '+&ieltsExamParticipantsId=' + row.id+'" >مشاهده نتیجه</a> ';
                    }
                },
            ],
            columnDefs: [{
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_IeltsExamParticipants')) {
        DT_IeltsExamParticipants.init();
    }
});
