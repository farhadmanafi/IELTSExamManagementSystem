var DT_IeltsExamResultList = function () {
    ...
    var initTable1 = function () {
        // begin first table
        var table = $('#DT_IeltsExamResultList').DataTable({
            responsive: true,
            // Pagination settings
            ...
            serverSide: true,
            ajax: {
                url: $('#DT_IeltsExamResultList').data("url"),
                type: 'POST'
            },
            columns: [
                { "className": "dt-center", "data": "title", "autowidth": true },
                {
                    ...
                    render: function (data, type, row) {
                        return '<a class="btn btn-warning font-weight-bold mr-2"' +
                            'href="/IeltsExamResult/IeltsExamParticipants_Load?Id=' + row.id + '" >مشاهده نتایج</a> ';
                    }
                },
            ],
            columnDefs: [{
...
jQuery(document).ready(function () {
    if (!$.fn.DataTable.isDataTable('#DT_IeltsExamResultList')) {
        DT_IeltsExamResultList.init();
    }
});
